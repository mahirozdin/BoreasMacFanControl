...
- **Overall status:** P0–P3 complete, P4 in progress. **The fan write path is real and proven on this machine:** the helper takes a fan over, drives it (999→1548 rpm in 0.5 s), hands it back, and every failure path returns the hardware to firmware — `kill -9` in 1.0 s, a frozen client via watchdog in 15.8 s, both measured with the fan actually spinning. P4 is closed: the manual slider drives the hardware through the safety chain with rpm-exact tracking, the state machine logs every transition, and `make smoke` reproduces the whole hardware proof in one command (sleep leg attended-only, release blocker B5). The engine's signal path is in: a curve that cannot be non-monotone, hysteresis whose lock is a proven plateau, an asymmetric rate limiter, and the step-model comparison that keeps ADR 0010 measurable. Profiles, triggers and arbitration are in — the active profile always carries the reason it is active. 133 tests, 10 gates. Next: P5.10.
...
| P5 | `IN_PROGRESS` | Control engine | **P5.10 — configuration schema** |
