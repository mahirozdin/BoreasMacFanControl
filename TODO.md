...
- **Overall status:** P0–P3 complete. **The privileged helper is installed on this machine and the dead man's switch is proven on real hardware:** `kill -9` hands the fans back in 0.03 s (connection invalidation), a frozen-but-alive client in 14.06 s (watchdog expiry), and the helper exits after every release so nothing privileged stays resident. 83 tests, 10 gates. Next: P4.01.
