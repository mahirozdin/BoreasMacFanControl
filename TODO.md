...
- **Overall status:** P0–P3 complete, P4 in progress. **The fan write path is real and proven on this machine:** the helper takes a fan over, drives it (999→1548 rpm in 0.5 s), hands it back, and every failure path returns the hardware to firmware — `kill -9` in 1.0 s, a frozen client via watchdog in 15.8 s, both measured with the fan actually spinning. 91 tests, 10 gates. Next: P4.04.
...
| P4 | `IN_PROGRESS` | Fan control and the safety chain | **P4.04 — K1 fan floor** |
