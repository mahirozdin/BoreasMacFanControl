...
- **Overall status:** P0–P3 complete, P4 in progress. **The fan write path is real and proven on this machine:** the helper takes a fan over, drives it (999→1548 rpm in 0.5 s), hands it back, and every failure path returns the hardware to firmware — `kill -9` in 1.0 s, a frozen client via watchdog in 15.8 s, both measured with the fan actually spinning. P4 is closed: the manual slider drives the hardware through the safety chain with rpm-exact tracking, the state machine logs every transition, and `make smoke` reproduces the whole hardware proof in one command (sleep leg attended-only, release blocker B5). 106 tests, 10 gates. Next: P5.01.
