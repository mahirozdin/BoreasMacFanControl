...
- **Overall status:** P0–P3 complete, P4 in progress. **The fan write path is real and proven on this machine:** the helper takes a fan over, drives it (999→1548 rpm in 0.5 s), hands it back, and every failure path returns the hardware to firmware — `kill -9` in 1.0 s, a frozen client via watchdog in 15.8 s, both measured with the fan actually spinning. 102 tests, 10 gates. The engine-side safety layers (K1-K3) are pure Core logic under undeletable invariant tests. Next: P4.08.
...
| P4 | `IN_PROGRESS` | Fan control and the safety chain | **P4.08 — manual control** |
