...
> **状态：尚未发布。** 软件功能已经完整，并在开发者的机器上运行——读取传感器、
> 通过特权助理驱动风扇，并在每一条失败路径上把风扇交还固件，全部在真实硬件上
> 测量过。缺的是**签名与公证**，没有它们 macOS 不会在你的 Mac 上运行特权助理。
> 在此之前唯一的途径是[从源码构建](#安装)。参见[路线图](#路线图)。
...
**目前还没有可安装的版本。** 有了以后，主渠道会是 Homebrew cask，备选是经过签名
和公证的 DMG。今天两者都不存在，在它们出现之前，本节就会这样写。
...
**我的 Mac 为什么这么热？**
通常是持续负载——编译、导出视频、运行虚拟机。Boreas 会显示芯片的哪个部分在发热，
让你能分辨是 CPU 在忙，还是出风口被挡住了。
